...
const initFinder = async (collection, preloader_modal_selector) => {
  ...
  exportButton =
    exportIt || new URLSearchParams(window.location.search).get("export");
...
const loadObject = async (location, data) => {
  if (!data) {
    ...
    if (validExports[location]["sortedBy"]) {
      ...
      options.params = { _sort: sort };
    }
    data = await fetchData(options);
    if (data.error) {
      const { error } = data;
      api.common.errorHandler({ endpoint, error });
      return w2uiDataGrid;
    }
  }
...
const recordsToExcel = (gridName) => {
  ...
  let records =
    w2ui[gridName].getSelection().length > 1
      ? w2ui[gridName]
          .getSelection()
          .map((v) => w2ui[gridName].records.filter((v1) => v1.recid === v)[0])
...
const preloaderConsultas = (data) => {
  ...
  for (let consulta of data) {
    if (
      ...
      consulta.creadoPor = consulta.createdBy
        ? consulta.createdBy.username
        : "";
      consulta.modificadoPor = consulta.updatedBy
        ? consulta.updatedBy.username
        : "";
      consulta.fecha = new Date(consulta.fecha).toLocaleDateString("es-AR", {
        timezone: "GMT-3",
      });
      consulta.fechaProximaConsulta = consulta.fechaProximaConsulta
        ? new Date(consulta.fechaProximaConsulta).toLocaleDateString("es-AR", {
            timezone: "GMT-3",
          })
        : "";
      consulta.fechaAbandonoCompromiso = consulta.fechaAbandonoCompromiso
        ? new Date(consulta.fechaAbandonoCompromiso).toLocaleDateString(
            "es-AR",
            { timezone: "GMT-3" }
          )
        : "";
      consulta.fechaAbandonoEfectiva = consulta.fechaAbandonoEfectiva
        ? new Date(consulta.fechaAbandonoEfectiva).toLocaleDateString("es-AR", {
            timezone: "GMT-3",
          })
        : "";
      consulta.createdAt = consulta.createdAt
        ...
        : "";
      consulta.updatedAt = consulta.updatedAt
        ? new Date(consulta.updatedAt).toLocaleDateString("es-AR", {
            timezone: "GMT-3",
          })
        : "";
      consulta.persona_primerConsulta = consulta.persona_primerConsulta
        ? new Date(consulta.persona_primerConsulta).toLocaleDateString(
            "es-AR",
            { timezone: "GMT-3" }
          )
        : "";
      consulta.persona_nacimiento = consulta.persona_nacimiento
        ? new Date(consulta.persona_nacimiento).toLocaleDateString("es-AR", {
            timezone: "GMT-3",
          })
        : "";
      consulta.persona_createdAt = consulta.persona_createdAt
        ? new Date(consulta.persona_createdAt).toLocaleDateString("es-AR", {
            timezone: "GMT-3",
          })
        : "";
      consulta.persona_updatedAt = consulta.persona_updatedAt
        ? new Date(consulta.persona_updatedAt).toLocaleDateString("es-AR", {
            timezone: "GMT-3",
          })
        : "";
...
const fixColumns = (consulta) => {
  return {
    ...
    persona_antecedentesPatologicos:
      consulta.persona_antecedentesPatologicos || "",
    persona_descripcionAntecedentesPatologicos:
      consulta.persona_descripcionAntecedentesPatologicos || "",
    persona_recibeMedicamentos: consulta.persona_recibeMedicamentos || "",
    persona_descripcionRecibeMedicamentos:
      consulta.persona_descripcionRecibeMedicamentos || "",
...
const cleanUpPersona = (rec) => {
  ...
  rec.imc = (rec.pesoKg && rec.alturaCm
    ? rec.pesoKg / Math.pow(rec.alturaCm / 100, 2)
    : ""
  )
    .toString()
    .split(".")
    .join(",");
  rec.creadoPor =
    rec.createdBy && rec.createdBy.username ? rec.createdBy.username : "";
  rec.modificadoPor =
    rec.updatedBy && rec.updatedBy.username ? rec.updatedBy.username : "";
  delete rec["0"];
  delete rec["__v"];
...
const validExports = {
  personas: {
    ...
    columns: [
      ...
      {
        field: "primerConsulta",
        caption: "Primer consulta",
        type: "date",
        format: "DD/MM/YYYY",
        size: "20%",
        sortable: true,
  ...
  consultas: {
    ...
    columns: [
      ...
      {
        field: "fechaAbandonoEfectiva",
        caption: "Fecha de abandono",
        size: "10%",
        sortable: true,
        type: "date",
        format: "DD/MM/YYYY",
      ...
      {
        field: "fechaProximaConsulta",
        caption: "Fecha próx. consulta",
        size: "10%",
        sortable: true,
        type: "date",
        format: "DD/MM/YYYY",
      ...
      {
        field: "horaProximaConsulta",
        caption: "Hora próx. consulta",
        size: "10%",
        sortable: false,
      },
    ],
